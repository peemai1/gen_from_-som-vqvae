...
def main():
    ...
    if args.mode == "train":
        ...
        torch.save(prior.state_dict(), "pixelcnn_prior_train.pt") #pixelcnn_train_img_prior.pt , pixelcnn_test_img_prior.pt
        print(f"[DEBUG] Using embedding_dim={args.embedding_dim}, n_embeddings={args.n_embeddings}")
        print("Prior saved to pixelcnn_prior_train.pt")

# ----------------- SAMPLE MODE -----------------
    elif args.mode == "sample":
        print("Loading model from pixelcnn_prior_train.pt...")
        ...
        prior.load_state_dict(torch.load("pixelcnn_prior_train.pt", map_location=device))
        prior.eval()
        ...
        torch.save(samples, "pixelcnn_samples_train.pt")
        print("Samples saved to pixelcnn_samples_train.pt")
        ...
        if args.vqvae_ckpt:
            ...
            with torch.no_grad():
                ...
                # Save to image grid
                save_image(recon, "pixelcnn_samples_train.png", nrow=4)
                print("Reconstructions saved to pixelcnn_samples_train.png")
        else:
